...
try:
    ...
    print("yliu PSI_Calc note: module 'pandas' is not installed; install it first")
    ERROR_flag += 1 
        
try:
    import math
except ImportError:
    print("yliu PSI_Calc note: module 'math' is not installed; install it first")    
    ERROR_flag += 1 
    

if ERROR_flag > 0 :
    print('yliu PSI_Calc note: modules missing. Return')
    ...
    def PSI_Calc(feature_list = []
        ...
        if len(data_cutdf_path) > 0 :
            ...
            orig_cutoff_pack = list(map(lambda x: orig_bin_cut_y(x, special_value_list_y(special_value_df, x), tile_pct, round_digit, 1.0, list(df_orig[x]), documented_cutoff_dict[x], list(df_orig[orig_wt])),feature_list))
            ...
            orig_cutoff_pack = list(map(lambda x: orig_bin_cut_y(x, special_value_list_y(special_value_df, x), tile_pct, round_digit, fixed_pct, list(df_orig[x]), [], list(df_orig[orig_wt])),feature_list))
            cutoff_df = pd.concat(orig_cutoff_pack)
        ...
        val_nobs = flatten_list_y([x[0] for x in values_pack])
        val_pct = flatten_list_y([x[1] for x in values_pack])
        val_psi = flatten_list_y([x[2] for x in values_pack])
        
        val_raw_new = flatten_list_y([x[3] for x in values_pack])
        val_raw_newpct = flatten_list_y([x[4] for x in values_pack])
        ...
        sub_cutoff['raw_newobs'] = val_raw_new
        sub_cutoff['raw_newpct'] = val_raw_newpct   
