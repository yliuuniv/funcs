...
def two_split(trend_flag, prev_bin_metrics, curr_bin_metrics, total_goods, total_bads, data_list,metrics_list,bin_info, metrics_name):
    ...
    if trend_flag == 'unknown':
        ...
        if trend_flag == 'Increasing':
            if (prev_bin_metrics < curr_bin_metrics) & (curr_bin_metrics < rest_all_popu_metrics):
                ...
                metrics_list, bin_info, data_list, stop_flag = two_split(trend_flag, prev_bin_metrics, curr_bin_metrics, total_goods, total_bads, data_list, metrics_list, bin_info, metrics_name)
...
def single_var_bin(variter, df_input, special_value, resp_var, metrics_name, total_wt_obs, total_wt_resp, total_wt_nonresp, wtd_tiledize, total_abs_obs, total_abs_resp):
    ...
    # missing
    n_wt_missing = local_copy_df[local_copy_df[variter].isna()]['localwtvar'].sum()
    n_abs_missing = local_copy_df[local_copy_df[variter].isna()].shape[0]
    
    n_wtresp_missing = local_copy_df[(local_copy_df[variter].isna()) & (local_copy_df[resp_var] ==1)]['localwtvar'].sum()
    n_wtnpresp_missing = n_wt_missing - n_wtresp_missing
    local_copy_df = local_copy_df.dropna()
...
,columns = ['varBin', 'var', 'type', 'sign', 'cutoff', 'wtd_obs', 'wtd_obspct', 'wtd_resp', 'wtd_resppct', 'abs_obs', 'abs_resp'])])
    ...
    if metrics_name.lower() == 'woe':
        bef_cont_list = cutoff_df.apply(lamnbda row: WOE((row.wtd_obs - row.wtd_resp), row.wtd_resp, total_wt_nonresp, total_wt_resp), axis = 1)

    if metrics_name.lower() == 'bin_resppct':
        bef_cont_list = cutoff_df.apply(lamnbda row: Bin_resprate((row.wtd_obs - row.wtd_resp), row.wtd_resp), axis = 1)
    ...
    if (remain_cumu_wt <= wtd_tilesize) | (local_copy_df.shape[0] == 1):
        ...
        , 'sign': ['ELSE'] \
        , 'cutoff': ['ELSE'] \
        , 'wtd_obs': remain_cumu_wt \
        , 'wtd_obspct': float(remain_cumu_wt/total_wt_obs) \
        ...
        rest_all_popu_metrics = metrics[-1]
        ...
        , 'abs_resp': local_copy_df[resp_var].sum()
            ...
            if data_list.shape[0] == 0 :
                ...
                abs_popu_l = list(bin_info['abs_obs'])
                abs_resp_l = list(bin_info['abs_resp'])
                ...
                abs_popu_l = list(bin_info['abs_obs']) + [data_list.shape[0]]
                abs_resp_l = list(bin_info['abs_resp']) + [data_list['abs_resp'].sum()]
